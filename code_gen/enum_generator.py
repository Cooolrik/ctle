...
from code_generator_helpers import formatted_output, write_lines_to_file
...
def generate_enum_declaration( enm:enum_type, out:formatted_output, name_padding:int = 0, value_padding:int = 0, pad_to_max:bool = True  ) -> None:
    ...
    for val in enm.values:
        ...
        if val.desc != None and len(val.desc) > 0:
            s += ' // ' + val.desc
        out.ln(s)
    out.end_block( add_semicolon=True )
...
def generate_enum_function_implementations( enm:enum_type, out:formatted_output, use_lowercase_strings:bool = False ) -> None:
    ...
    for val in enm.values:
        if use_lowercase_strings:
            out.indented_ln(f'{{ {val.name} , "{val.name.lower()}" }},')
        else:
            out.indented_ln(f'{{ {val.name} , "{val.name}" }},')
    out.end_block( add_semicolon=True )
    out.ln()
    ...
    for val in enm.values:
        if val.desc != None:
            out.indented_ln(f'{{ {val.name} , "{val.desc}" }},')
    out.end_block( add_semicolon=True )
    out.ln()    
    ...
    for val in enm.values:
        if use_lowercase_strings:
            out.indented_ln(f'{{ "{val.name.lower()}" , {val.name} }},')
        else:
            out.indented_ln(f'{{ "{val.name}" , {val.name} }},')
    out.end_block( add_semicolon=True )
    out.ln()   
