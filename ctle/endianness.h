...
/// @details Template specialization is implemented for uint16_t, uint32_t, and uint64_t.
...
template <class T> T from_bigendian( const uint8_t *src );
...
template <> inline uint16_t from_bigendian<uint16_t>( const uint8_t *src )
{
...
template <> inline uint32_t from_bigendian<uint32_t>( const uint8_t *src )
{
    return ( uint32_t( from_bigendian<uint16_t>( &src[0] ) ) << 16 ) | uint32_t( from_bigendian<uint16_t>( &src[2] ) );
}
...
template <> inline uint64_t from_bigendian<uint64_t>( const uint8_t *src )
{
    return ( uint64_t( from_bigendian<uint32_t>( &src[0] ) ) << 32 ) | uint64_t( from_bigendian<uint32_t>( &src[4] ) );
}
...
template <class T> void to_bigendian( uint8_t *dst, T value );
...
template <> inline void to_bigendian<uint16_t>( uint8_t *dst, uint16_t value )
{
...
template <> inline void to_bigendian<uint32_t>( uint8_t *dst, uint32_t value )
{
    to_bigendian<uint16_t>( &dst[0], uint16_t( ( value >> 16 ) & 0xffff ) );
    to_bigendian<uint16_t>( &dst[2], uint16_t( value & 0xffff ) );
}
...
template <> inline void to_bigendian<uint64_t>( uint8_t *dst, uint64_t value )
{
    to_bigendian<uint32_t>( &dst[0], uint32_t( ( value >> 32 ) & 0xffffffff ) );
    to_bigendian<uint32_t>( &dst[4], uint32_t( value & 0xffffffff ) );
}
