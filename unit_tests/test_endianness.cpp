...
template<class _Ty> void test_bigendian_from_value( _Ty value, uint8_t *expected )
{
	...
	to_bigendian<_Ty>( big_endian_dest, value );
...
TEST( endianness, basic_test )
{
	...
	EXPECT_EQ( from_bigendian<uint16_t>( big_endian ), ( (uint16_t)0x1234 ) );
	EXPECT_EQ( from_bigendian<uint32_t>( big_endian ), ( (uint32_t)0x12345678 ) );
	EXPECT_EQ( from_bigendian<uint64_t>( big_endian ), ( (uint64_t)0x123456789abcdef0 ) );
