...
inline void setup_random_seed() {
    i64 seed =
        (global_random_seed == -1) ? i64(time(nullptr)) : global_random_seed;
...
inline u32 u32_rand() {
    return u32(u16_rand()) << 16 | u32(u16_rand());
}
inline u64 u64_rand() {
...
template <>
i16 random_value<i16>() {
    return u16_rand();
}
template <>
i32 random_value<i32>() {
    return u32_rand();
}
template <>
i64 random_value<i64>() {
    return u64_rand();
}
   ...
   private:
    ...
    void run() {
        ...
        for (size_t lev = 0; lev < levelsToTest; ++lev) {
            ...
            this->iterations_per_thread =
                this->passCount_thread_safe_map / num_threads;
            ...
            double timeSpan_s =
                std::chrono::duration_cast<std::chrono::duration<double>>(t2 -
                                                                          t1)
                    .count();
            double perIterationTime_us =
                timeSpan_s / double(num_threads) /
                double(itemCount_thread_safe_map * passCount_thread_safe_map) *
                double(1000000);

            std::cout << "\tRunning " << num_threads << " threads took "
                      << timeSpan_s
                      << " secs.\tTime per iteration: " << perIterationTime_us
                      << " usecs." << std::endl;
        }
    }
